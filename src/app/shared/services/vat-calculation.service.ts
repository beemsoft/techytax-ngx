...
export class VatCalculationService {
  ...
  static applyFixedAmount(transaction:Transaction, fixedAmount:number): Transaction {
    ...
    if (transaction.costMatch.vatType === VatType.HIGH) {
      transaction.amountVat = Math.round(fixedAmount * 21) / 100;
    } else if (transaction.costMatch.vatType === VatType.LOW) {
  ...
  calculateTotalVat(transactions:Array<Transaction>): Observable<VatReport> {
    ...
    for (let i = 0; i < transactions.length; i++) {
      if (transactions[i].costCharacter === CostCharacter.IGNORE) {
        transactions[i].amountNet = 0;
        transactions[i].amountVat = 0;
      } else {
        let vatIn = 0, vatOut = 0, transaction:Transaction;
        switch (transactions[i].costType["id"]) {
          ...
          default:
            if (transactions[i].costMatch != null && transactions[i].costMatch.vatType != null) {
              ...
              if (transactions[i].costType["id"] === CostType.BUSINESS_CAR) {
                totalCarCosts += transaction.amountNet;
              } else if (transactions[i].costType["id"] === CostType.TRAVEL_WITH_PUBLIC_TRANSPORT) {
                totalTransportCosts += transaction.amountNet;
              } else if (transactions[i].costType["id"] === CostType.OFFICE) {
                totalOfficeCosts += transaction.amountNet;
              } else if (transactions[i].costType["id"] === CostType.GENERAL_EXPENSE) {
